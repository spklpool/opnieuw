...
namespace Opnieuw.Parsers.CSParser
{
	public class LockStatement : Statement
	{
		public new static Statement parse(TokenProvider tokenizer)
		{
			...
			if (Token.LOCK == tokenizer.CurrentToken.Type)
			{
				PositionToken lockToken = tokenizer.CurrentToken;
				tokenizer.nextToken(); // lock
				if (Token.OPEN_PARENS == tokenizer.CurrentToken.Type)
				{
					...
					if (false == (expression is MissingExpression))
					{
						if (Token.CLOSE_PARENS == tokenizer.CurrentToken.Type)
						{
							...
							{
								ret = new LockStatement(lockToken, openParensToken, expression, closeParensToken, statement);
							}
						}
					}
				}
		...
		public LockStatement(PositionToken lockToken, PositionToken openParensToken, Expression expression, PositionToken closeParensToken, Statement statement) :
		base(lockToken, openParensToken, expression, closeParensToken, statement)
		{
		}
		
		public PositionToken LockToken {
			get {
